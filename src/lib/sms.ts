 ...
 */
export async function storeOTP(phoneNumber: string, otp: string, expiryMinutes: number = 10): Promise<boolean> {
  if (!redis) {
 ...
 */
export async function canSendOTP(phoneNumber: string, cooldownSeconds: number = 60): Promise<{ canSend: boolean; remainingTime: number }> {
  if (!redis) {
 ...
 */
export async function verifyPhoneNumber(phoneNumber: string, otp: string): Promise<{
  success: boolean
  message: string
}> {
