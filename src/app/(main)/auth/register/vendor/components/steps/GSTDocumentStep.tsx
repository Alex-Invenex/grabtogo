...
import { Eye, Shield, Info, Loader2, CheckCircle, MapPin, Calendar, FileText, Upload, X, Check } from 'lucide-react'
...
export default function GSTDocumentStep() {
  ...
  const handleFileSelect = (file: File | null) => {
    ...
    if (file.size > maxSize) {
      setUploadError(`File size must be less than 5MB. Current size: ${(file.size / (1024 * 1024)).toFixed(2)}MB`)
      return
    }
        ...
        <div className="space-y-6">
                      ...
                      <div className="relative p-4 border-2 border-green-300 rounded-xl bg-green-50">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            {preview ? (
                              <img src={preview} alt="GST Certificate" className="w-16 h-16 object-cover rounded-lg" />
                            ) : (
                            ...
                            <div>
                              <p className="font-medium text-gray-900">{(gstCertificate as File).name}</p>
                              <p className="text-sm text-gray-500">
          ...
          <div className="p-4 bg-primary/5 rounded-lg border border-primary/20">
            <p className="text-sm text-primary/80">
              <strong>Note:</strong> Upload a clear copy of your GST certificate. This document will be verified by our team to ensure authenticity.
            </p>
          </div>
        </div>
      )}
