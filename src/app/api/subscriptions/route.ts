...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userRole !== 'VENDOR') {
      return NextResponse.json({ error: 'Only vendors can create subscriptions' }, { status: 403 });
    }
  ...
  try {
    ...
    const usage = {
      ...
      orders: {
        ...
        percentage:
          subscription.maxOrders > 0 ? Math.round((orderCount / subscription.maxOrders) * 100) : 0,
      },
    };
