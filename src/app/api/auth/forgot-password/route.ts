  ...
  try {
    ...
    if (user && user.emailVerified && user.isActive) {
      ...
      await logSecurityEvent({
        ...
        details: JSON.stringify({ email, reason: !user ? 'user_not_found' : !user.emailVerified ? 'email_not_verified' : 'account_inactive' }),
        ipAddress,
        userAgent: request.headers.get('user-agent') || 'unknown',
      })
    }
    ...
    return NextResponse.json(
      {
        message: 'If an account with that email exists and is verified, a password reset email will be sent.',
        email: email
      },
      { status: 200 }
    )
